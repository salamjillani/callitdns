...
const getStripe = () => {
  if (!stripe) {
    ...
    if (!secretKey) {
      throw new Error(
        "Stripe secret key not configured. Please set STRIPE_SECRET_KEY in environment variables."
      );
    }
...
const PLANS = {
  ...
  pro: {
    name: "Pro",
    priceId:
      process.env.STRIPE_PRO_PRICE_ID || "price_1RvOH2R1JkzRn27a7Ru5ZHv0",
    price: 19,
    features: {
      domains: 10,
      scansPerMonth: 100,
  ...
  enterprise: {
    ...
    priceId:
      ...
      "price_1RvOKWR1JkzRn27aoCcblTD2",
    price: 99,
    features: {
      domains: -1, // unlimited
      scansPerMonth: -1,
