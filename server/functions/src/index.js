...
const verifyAuth = (request) => {
  ...
  if (request && request.auth) {
    console.log("Auth object details:", {
      ...
      email: request.auth.token
        ? request.auth.token.email
        : "no email in token",
      emailVerified: request.auth.token
  ...
  async (request) => {
    ...
    console.log("Environment check:", {
      ...
      cloudflareKeyPreview: process.env.CLOUDFLARE_API_KEY
        ? `${process.env.CLOUDFLARE_API_KEY.substring(
            0,
            5
          )}...${process.env.CLOUDFLARE_API_KEY.slice(-5)}`
        : "missing",
  ...
  async (request) => {
    ...
    try {
      ...
      try {
        ...
        dottyResponse = await processDottyCommand(
          command,
          domain,
          existingRecords
        );
        ...
        // Execute the actions
        results = await executeDottyActions(
          dottyResponse.actions,
          domain,
          zoneId
        );
      } catch (moduleError) {
      ...
      try {
        switch (event.type) {
          ...
          case "customer.subscription.updated":
            ...
            const userSnapshot2 = await usersRef2
