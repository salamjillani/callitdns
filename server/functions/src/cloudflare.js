...
function getCloudflareConfig() {
  const config = {
    apiKey:
      process.env.CLOUDFLARE_API_KEY || "eca97f36f2d33f3225d70ba2de4ab6f20e50c",
    email: process.env.CLOUDFLARE_EMAIL || "amanueldagnamyelew@gmail.com",
    zoneId:
      process.env.CLOUDFLARE_ZONE_ID || "04afc7b35b0d74949591940dc43cc0eb",
  };
...
async function getZoneId(domain) {
  ...
  if (!apiKey || !email) {
    throw new Error(
      "Cloudflare credentials not configured. Please set CLOUDFLARE_API_KEY and CLOUDFLARE_EMAIL in environment variables."
    );
  }
  ...
  try {
    ...
    console.error(
      "Error getting zone ID. Full error:",
...
async function getDNSRecords(domain) {
  ...
  try {
    ...
    console.log(
      `Successfully fetched ${response.data.result.length} DNS records`
    );
