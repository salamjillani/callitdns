...
import { doc, getDoc, updateDoc, increment, collection, query, where, limit, onSnapshot } from 'firebase/firestore';
    ...
    return content
      ...
      .map((paragraph, index) => {
        ...
        if (paragraph.startsWith('# ')) {
          return <h2 key={index} className="text-xl sm:text-2xl font-bold text-white mt-6 mb-4">{paragraph.substring(2)}</h2>;
        }
        if (paragraph.startsWith('## ')) {
          return <h3 key={index} className="text-lg sm:text-xl font-semibold text-white mt-4 mb-3">{paragraph.substring(3)}</h3>;
        }
        if (paragraph.startsWith('### ')) {
          return <h4 key={index} className="text-base sm:text-lg font-semibold text-white mt-3 mb-2">{paragraph.substring(4)}</h4>;
        }
        ...
        if (paragraph.startsWith('```')) {
          ...
          return (
            <pre key={index} className="bg-slate-800/50 border border-slate-700 rounded-lg p-4 mb-4 overflow-x-auto">
        ...
        // Regular paragraph
        return <p key={index} className="text-slate-300 mb-4 leading-relaxed text-sm sm:text-base">{paragraph}</p>;
      });
  };
    ...
    return (
      <Layout>
        <div className="py-12 text-center px-4">
          <h1 className="text-xl sm:text-2xl font-bold text-white mb-4">Post Not Found</h1>
  ...
  return (
    <Layout>
      <div className="max-w-4xl mx-auto py-8 sm:py-12 px-4 sm:px-6">
        {/* Back Button */}
        <Link to="/blog" className="inline-flex items-center space-x-2 text-slate-400 hover:text-white mb-6 sm:mb-8 transition">
        ...
        <article>
          <header className="mb-6 sm:mb-8">
            ...
            <p className="text-base sm:text-lg text-slate-300 mb-4 sm:mb-6 leading-relaxed">{post.excerpt}</p>
          ...
          {post.tags && post.tags.length > 0 && (
            <div className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-slate-800">
              <div className="flex items-center space-x-2 mb-4">
                ...
                <span className="text-slate-400 text-sm sm:text-base">Tags:</span>
              </div>
              <div className="flex flex-wrap gap-2">
                {post.tags.map((tag, index) => (
                  <span key={index} className="px-3 py-1 bg-slate-800 text-slate-300 rounded-lg text-xs sm:text-sm">
                    #{tag}
                  </span>
                ))}
              </div>
        ...
        {relatedPosts.length > 0 && (
          <div className="mt-8 sm:mt-12 pt-8 sm:pt-12 border-t border-slate-800">
            <h2 className="text-xl sm:text-2xl font-bold text-white mb-6">Related Posts</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {relatedPosts.map((relatedPost) => (
                ...
                >
                  ...
                  <h3 className="font-semibold text-white mb-2 text-sm sm:text-base line-clamp-2">{relatedPost.title}</h3>
                  <p className="text-xs sm:text-sm text-slate-400 line-clamp-2">{relatedPost.excerpt}</p>
